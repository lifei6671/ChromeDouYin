...
func main() {
	...
	//u := "ws://127.0.0.1:9222/devtools/browser/025c0832-08f1-457f-829f-9b9052dc5b44"
	...
	err = web.Run(config.Default.Addr, func() *chrome.Chrome {
		log.Printf(" [GIN] cookie=[%s]", config.Default.Cookie)
		log.Printf(" [GIN] service-url=[%s]", config.Default.ServiceURL)
		return chrome.New(chrome.WithCookie(config.Default.Cookie), chrome.WithServiceURL(config.Default.ServiceURL))
	})
	if err != nil {
		log.Fatalln(err)
	}
